...
CREATE TABLE IF NOT EXISTS `bitacora_db`.`topic` (
  ...
  `files_id` CHAR(36) NULL,
  `created_at` TIMESTAMP NOT NULL,
  `updated_at` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_topic_comment1_idx` (`comment_id` ASC) VISIBLE,
  INDEX `fk_topic_files1_idx` (`files_id` ASC) VISIBLE,
  CONSTRAINT `fk_topic_comment1`
    FOREIGN KEY (`comment_id`)
    REFERENCES `bitacora_db`.`comment` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_topic_files1`
    FOREIGN KEY (`files_id`)
    REFERENCES `bitacora_db`.`files` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
